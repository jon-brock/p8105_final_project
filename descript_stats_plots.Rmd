...
THIS IS NOT TRUE DO NOT INCLUDE:
...
nyc_plots %>%
    select(area_name, borough, open_complaint) %>% 
    group_by(area_name, borough, open_complaint) %>%
    summarize(n = n()) %>% 
    pivot_wider(names_from = open_complaint, values_from = n) %>% 
    rename(closed = `0`,
           open = `1`) %>% 
    mutate(proportion_open = open/(open + closed)) %>% 
    filter(!is.na(area_name), !is.na(proportion_open)) %>% 
    ggplot(aes(x = area_name, y = proportion_open, fill = borough)) + 
    geom_col() + 
    facet_wrap(~borough, scales = "free_x") + 
    theme(axis.text.x = element_text(angle = 75, hjust = 1)) +
    xlab("Neighborhood") +
    ggtitle("Proportion of Complaints that are Open per Neighborhood") +
    ylab("Proportion Open") +
    theme(legend.position = "none") +
    scale_fill_viridis(discrete = TRUE)
```
...
## Differences in open cases by race
...
#### Looking at total cases still open
...
#### Then looking at proportion of cases
