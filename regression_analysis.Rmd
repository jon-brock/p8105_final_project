...
nycedit_cb2 = add_inc %>% 
    ...
    select(community_board, number_complaints, median_income, num_unsolved, num_health_complaint, year) %>% 
    distinct() %>% 
    arrange(community_board)

nyc_cb2 = glm(num_health_complaint ~ median_income + num_unsolved + year, data = nycedit_cb2)
summary(nyc_cb2)



nyc_model = glm(open_complaint ~ median_income + year + health_complaint, data = add_inc)
